...
export default function App() {
  ...
  const subscription = useSubscription(
    session.status === 'signed-in' ? business?.ownerId ?? null : null,
  );
  ...
  const completeAndScheduleNext = useMutation(
    api.recurrence.completeAndScheduleNext,
  );
  const createFromTemplate = useMutation(
    api.requirementsMutations.createFromTemplate,
  );
  ...
  const usingSampleData =
    previewOnly || !business || liveRequirements === undefined;
  const items: Requirement[] = usingSampleData
    ? SAMPLE_REQUIREMENTS
    : liveRequirements;
  ...
  const watchSnapshot = useMemo(
    () =>
      makeWatchSnapshot(
        session.status === 'signed-in' ? business : null,
        usingSampleData ? [] : items,
        locale,
      ),
  ...
  useEffect(() => {
    watchBridge
      ?.updateSnapshot(JSON.stringify(watchSnapshot))
      .catch(() => undefined);
  }, [watchSnapshot]);
  ...
  const saveRequirement = useCallback(
    async (item: NewRequirement) => {
      ...
      try {
        await scheduleReminderForDueDate(
          item.title,
          item.dueDate,
          requirementId,
        );
  ...
  const adoptTemplate = useCallback(
    async (rule: RuleTemplate, dueDate: string) => {
      if (!business) throw new Error(copy.signInRequired);
      const requirementId = await createFromTemplate({
        businessId: business._id,
        ruleId: rule._id,
        dueDate,
        locale,
  ...
  const completeRequirement = useCallback(
    async (item: Requirement) => {
      ...
      if (nextId && item.recurrence) {
        const next = await convex.query(api.requirements.list, {
          businessId: business!._id,
        });
      ...
      try {
        ...
        for (const action of actions) {
          ...
          const item = items.find(
            candidate => candidate._id === action.requirementId,
          );
          if (!error && item?.recurrence && !subscription.isPlus)
            error = copy.recurrenceIsPlus;
          if (!error && item?._id) {
            try {
  ...
  const viewEvidence = useCallback(
    async (item: Requirement) => {
      if (!item._id) return;
      const url = await convex.query(api.documents.getEvidenceUrl, {
        requirementId: item._id,
      });
      if (!url) throw new Error(copy.noEvidence);
      const opened = await Linking.canOpenURL(url);
      if (!opened)
        throw new Error('No app on this device can open that document.');
  ...
  const deleteAccount = useCallback(async () => {
    const finishDeletion = async () => {
      try {
        ...
        await Promise.all(
          removed.requirementIds.map(cancelReminderForRequirement),
        );
  ...
  const sendTestReminder = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        copy.text('Test reminder scheduled'),
        copy.text(
          'Lock your phone. A test reminder should arrive in about one minute.',
        ),
      );
    } catch (error) {
      setActionError(message(error));
  ...
  const checkTestReminder = useCallback(async () => {
    try {
      ...
      Alert.alert(
        copy.text('Test reminder status'),
        copy.text(
          delivered
  ...
  if (profileOpen && business) {
    return (
      <Onboarding
        ...
        initial={{
          name: business.name,
          country: business.country as BusinessProfile['country'],
          region: business.region,
          industry: business.industry,
          locality: business.locality,
          entityType: business.entityType,
        ...
        onCreate={async profile => {
          ...
          try {
            await updateProfile({ businessId: business._id, ...profile });
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScreenScroll>
        <View style={s.top}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              setLocale(
                localeOrder[
                  (localeOrder.indexOf(locale) + 1) % localeOrder.length
                ],
              )
            }
          >
        ...
        {actionError && (
          <ErrorBanner
            ...
            retryLabel={copy.close}
          />
        )}
        {!subscription.isPlus &&
          !usingSampleData &&
          items.length >= FREE_REQUIREMENT_LIMIT && (
            <TouchableOpacity
              accessibilityRole="button"
              style={s.banner}
        ...
        {tab === 'home' && (
          <Home
            ...
            onBrowseTemplates={
              business ? () => setTemplatesOpen(true) : undefined
            }
          />
        )}
        {tab === 'calendar' && (
      ...
      {selected && !editing && (
        <RequirementDetail
          ...
          onComplete={
            selected.recurrence ? completeRequirement : markStatusCurrent
          }
      ...
      <View style={[s.nav, { bottom: insets.bottom }]}>
        {TABS.map(({ key, icon, label }) => (
          ...
          >
            ...
            <Text style={[s.navLabel, tab === key && s.navActive]}>
              {copy.text(label)}
            </Text>
          </TouchableOpacity>
        ))}
